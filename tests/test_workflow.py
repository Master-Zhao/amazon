"""End-to-end bounded workflow tests for all required paths."""
...
def test_high_acos_waits_for_approval() -> None:
...
def test_insufficient_evidence_completes_without_change() -> None:
    output = run_workflow(load_example("insufficient-evidence.json")).output
...
def test_no_change_required_completes_without_change() -> None:
    output = run_workflow(load_example("no-change-required.json")).output
...
def test_invalid_once_revises_then_succeeds() -> None:
...
def test_plan_version_increments_after_revision() -> None:
    output = run_workflow(load_example("invalid-reasoner-output.json"), reasoner_mode="invalid_once").output
    assert output["plan_version"] == 2 and output["retry_count"] == 1


def test_attempt_id_changes_after_revision() -> None:
    task = load_example("invalid-reasoner-output.json")
    original = task["attempt_id"]
    output = run_workflow(task, reasoner_mode="invalid_once").output
    assert output["attempt_id"] != original


def test_same_error_twice_stops_automatic_revision() -> None:
...
class ChangingInvalidReasoner:
    """Test-only sequence that avoids the same-error stop until max revisions."""
...
def test_automatic_revisions_never_exceed_three() -> None:
...
class CountingValidReasoner:
    """Test-only valid Reasoner that exposes its invocation count."""
...
def test_validation_success_stops_further_reasoner_calls() -> None:
...
def test_preflight_passes_without_production_write() -> None:
    preflight = run_workflow(load_example("high-acos-keyword.json")).output["execution_preflight"]
    assert preflight["passed"] is True
    assert preflight["production_write_called"] is False


def test_every_terminal_path_keeps_production_write_false_or_absent() -> None:
    cases = (
    ...
    for filename, mode in cases:
        preflight = run_workflow(load_example(filename), reasoner_mode=mode).output["execution_preflight"]
        assert preflight is None or preflight["production_write_called"] is False


def test_workflow_does_not_access_network(monkeypatch) -> None:
    def blocked_socket(*args, **kwargs):
        raise AssertionError("network access is forbidden")

    monkeypatch.setattr(socket, "socket", blocked_socket)
    assert run_workflow(load_example("high-acos-keyword.json")).output["current_status"] == "waiting_for_approval"


def test_workflow_does_not_start_development_tests(monkeypatch) -> None:
...
def test_key_workflow_steps_are_audited() -> None:
    events = {event["event_type"] for event in run_workflow(load_example("invalid-reasoner-output.json"), reasoner_mode="invalid_once").audit_events}
    required = {
...
def test_audit_step_ids_are_unique() -> None:
    events = run_workflow(load_example("high-acos-keyword.json")).audit_events
