...
## 2.6 两个远程广告数据库只读接入（2026-07-30）

确认依据：项目发起人明确要求项目连接现有 SCM 与广告分析数据库读取真实数据，
并完成前后端联调。该授权仅覆盖只读集成，不授权修改远程库、执行远程迁移或把
远程数据直接暴露给前端。

| ID | 状态 | 决策 | 说明 |
|---|---|---|---|
| D-153 | 已确认 | `default` 始终保留为项目自有 MySQL；远程库使用 `scm_remote` 与 `ads_analysis_remote` 独立别名 | 登录、权限、审计和正式状态不落入既有远程业务库 |
| D-154 | 已确认 | 两个远程别名禁止 Django 迁移，并要求生产使用仅有 `SELECT` 权限的账号 | 代码查询同样只实现固定参数化 `SELECT` |
| D-155 | 已确认 | 通过本地环境变量把 `AdvertisingProfile.external_profile_id` 映射到远程 `mer_id` + 精确 `mer_code` | 对用户提供的 `eb_ad_campaign.json` 完整聚合确认：`mer_id` 与 `mer_code` 均非全局一一对应；查询必须同时过滤两列，且不提供普通成员任意选择商户的接口 |
| D-156 | 已确认 | 远程 Campaign 页面使用分析库事实并以 SCM Campaign 元数据补充名称、状态和预算 | 页面显式标识 `REMOTE_MYSQL` 与 SCM 是否匹配，不冒充项目导入后的权威 Daily Metric |
| D-157 | 已确认 | 远程库不提供 currency 时使用已授权 Profile 的 currency，仅在单一 Profile 内展示，不跨币种汇总 | 必须由可信运维确保 Profile 与远程商户/站点映射正确 |
| D-158 | 已确认 | 未指定日期时读取映射商户最新数据日，单次返回上限可配置且默认 200 | 防止对百万级分析表执行无界查询 |
| D-159 | 已确认 | 独立远程运行模式使用项目自有远程系统库作为 `default`，并以 `scm_remote` 只读连接既有 SCM 库 | 系统库仍承载登录、权限、Token、Session 与审计；SCM 库禁止迁移、建表、跨库外键和业务写入 |

## 2.7 登录账号标识兼容（2026-07-30）

确认依据：项目发起人明确要求字母数字账号（如 `W0765`）不得被前端邮箱格式
校验拒绝。本次只扩展本项目系统用户的登录标识契约，不解除远程 SCM 用户映射、
密码算法和系统数据库可用性门槛。

| ID | 状态 | 决策 | 说明 |
|---|---|---|---|
| D-160 | 已确认 | 登录推荐字段改为 `identifier`，支持本地 `User.username` 或大小写不敏感的 `User.email`；旧 `email` 字段暂时兼容 | 前端使用文本账号输入且不做邮箱格式强制；两字段同时提交或出现未知字段时明确拒绝；不自动创建远程用户映射 |

## 2.8 远程 SCM 商户账号认证（2026-07-31）

确认依据：项目发起人明确要求已连接远程数据库后，远程库账号能够从当前前端正常
登录。本节覆盖 D-160 中“不自动创建远程用户映射”的旧边界，但不改变 D-153/D-154
的远程库只读约束。

| ID | 状态 | 决策 | 说明 |
|---|---|---|---|
| D-161 | 已确认 | 启用后，登录服务可对 `scm_remote.eb_merchant_admin` 执行固定参数化只读查询并校验其 bcrypt 密码；首次成功时在项目 `default` 库创建 `sys_user` 和 `sys_external_identity` 映射 | 不复制远程密码；映射用户使用不可用本地密码；JWT、Refresh Token、审计和所有正式状态仍只写项目系统库 |
| D-162 | 已确认 | 本地账号优先且禁止远程同名账号接管；远程账号必须精确匹配、唯一、`status=1` 且 `is_del=0` | 远程源不可用返回 503；账号不存在与密码错误仍统一返回 401 |
| D-163 | 已确认 | 远程账号认证成功不自动授予 Tenant、Store 或 Profile 权限 | 管理员必须在项目系统库显式建立 Membership/授权，避免仅凭外部账号存在扩大数据范围 |

## 2.9 Access Token 请求标头兼容（2026-07-31）

确认依据：项目发起人明确要求前端请求在浏览器开发者工具中携带 `X-Token`。

| ID | 状态 | 决策 | 说明 |
|---|---|---|---|
| D-164 | 已确认 | 前端对已有 Access Token 的请求同时发送标准 `Authorization: Bearer <token>` 和兼容 `X-Token: <token>` | Token 仍只保存在内存；Refresh Token 仍只使用 HttpOnly Cookie |
